...
const actualizarUsuario = async(_, { input }, validacion) => {
 ...
 /**
  ...
  if(usr.rol !== "admin_usuario" && usuario_login !== id_usuario){
      throw new Error("Usted no cuenta con los permisos necesarios para realizar esta operación");
  }
  ...
  const actualizacionUsuario = await Usuario.findByIdAndUpdate(id_usuario, cambiosUsuario, {new: true});
